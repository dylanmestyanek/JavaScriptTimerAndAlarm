...
function adjustTimer(e: MouseEvent){
    // If 'Stop' button is pressed, clear timer, timer displays, and hide Play/Pause/Stop buttons
    if ((e.target as HTMLButtonElement).dataset.method === 'stop') {
        endTimer();
        clearInterval(countdown);
        timerDisplay.classList.remove("paused");
        timerDisplay.textContent = '';
    ...
    if ((e.target as HTMLButtonElement).dataset.method === 'pause') {
        clearInterval(countdown);
    ...
    if ((e.target as HTMLButtonElement).dataset.method === 'play' && timerDisplay.classList.contains("paused")) {
